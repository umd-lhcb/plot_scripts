...
int main(){
  ...
  string ntpDst = "ntuples/0.9.17-all_years/2016/Dstst_heavy/DststHMuDst-11676012-MagDown/D0--25_12_02--mc--11676012--2016--md--tracker_only.root";
  ...
  procs.push_back(Process::MakeShared<Baby_rdx917>
                  ("D**(2580) #rightarrow D^{*+}", Process::Type::background, colors("purple"),
                   set<string>({ntpDst}), globalCuts + " && truthmatch%100 == 10",2)); 
  procs.push_back(Process::MakeShared<Baby_rdx917>
                  ("D**(2640) #rightarrow D^{*+}", Process::Type::background, colors("red"),
                   set<string>({ntpDst}), globalCuts + " && truthmatch%100 == 20",2)); 
  procs.push_back(Process::MakeShared<Baby_rdx917>
                  ("D**(2737) #rightarrow D^{*+}", Process::Type::background, colors("green"),
                   set<string>({ntpDst}), globalCuts + " && truthmatch%100 == 30",2)); 
  procs.push_back(Process::MakeShared<Baby_rdx917>
                  ("D**(2978) #rightarrow D^{*+}", Process::Type::background, colors("darkblue"),
                   set<string>({ntpDst}), globalCuts + " && truthmatch%100 == 40",2)); 
  ...
  pm.Push<Hist1D>(Axis(50,2100,3500, "d_iso1_iso2_m", "m(D^{0}#pi) [GeV]"), "wskim_2os", procs, linplot);
