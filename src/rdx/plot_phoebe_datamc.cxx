...
// Should dssmu && dsstau and ddmu && ddtau be grouped together (just dss and dd types)?
enum class eventType {data, dsptau, dspmu, dssmu, dsstau, ddtau, ddmu, unknown}; // ADD ANY OTHER EVENT TYPES HERE
...
int main(){
  ...
  NamedFunc weight("weight",[&](const Baby &b){
                              ...
                              if(type == eventType::data) return 1.;
                                                                     ...
                                                                     // define individual weight functions, even
                            });
  ...
  // ADD ANY OTHER PROCESSES HERE, eg. MC B->D** tau nu, AS BACKGROUND PROCESS TYPES
  ...
  // Are reaction descriptions for ddtau and ddmu modes correct? I'm basing on table 18 pg 91 of ANA note
  procs.push_back(Process::MakeShared<Baby_phoebe_dsp>("B #rightarrow D*^{+}(D_{s}^{-} #rightarrow #tau #nu) X",
                                                       ...
                                                       set<string>({repofolder+ntuplefile}),
                                                       event_type == static_cast<double>(eventType::ddtau)));
  procs.push_back(Process::MakeShared<Baby_phoebe_dsp>("B #rightarrow D*(X_{c} #rightarrow #mu #nu X') X",
                                                       ...
                                                       set<string>({repofolder+ntuplefile}),
                                                       event_type == static_cast<double>(eventType::ddmu)));
  // include processes for D** feed down
