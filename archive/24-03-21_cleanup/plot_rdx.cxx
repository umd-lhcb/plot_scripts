...
int main(){
  ...
  procs.push_back(Process::MakeShared<Baby_run2_std>("D*^{+}#mu#nu MC 2016", Process::Type::background, colors("green"),
  ...
  //pm.Push<Hist1D>(Axis(100,0,20, "b_pt", "p_{T}(B^{0}) [GeV]"), "mm2<0.5", procs, linplot, weights).RatioTitle("Data", "MC").SetTitle("m^{2}_{miss} < 0.5 GeV^{2}").Tag("mc");
  pm.Push<Hist1D>(Axis(100,5,12.5, logbpt, "log(B^{0} p_{T} [MeV])"), "mm2<0.5", procs, linplot, weights).RatioTitle("Data", "MC").SetTitle("m^{2}_{miss} < 0.5 GeV^{2}").Tag("mc");
